...
> ADRs live under `meta/`, not the canonical `docs/adr/`. `docs/` is the published Starlight site and nothing else — it is a build directory, so anything under it ships to auberge.espadat.com. Internal material — ADRs, throwaway plans under `meta/agents/plans/`, research — stays in `meta/`.
