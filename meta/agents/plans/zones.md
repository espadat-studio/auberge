...
> - **A named Zone also needs config to declare the Host serves the App**, so this phase changed `zone.rs` where the plan had it Ansible-only. A Meta's `zone:` holds on every Host, so the pin alone put the agent tier's token on all three boxes. `declared_on_host` reads the App's serving gate or its `<app>_zone`, both through `hosts::gate_answered` — the one spelling ADR-0083 settled, rebased onto on 2026-09-22. A stricter reading of the same keys (the Host's own table alone) was written first and dropped: two expressions of one rule is the divergence ADR-0081 deletes, and a fleet-wide `<app>_subdomain` widening a Zone's reach is what answering it fleet-wide says.
