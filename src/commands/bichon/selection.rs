...
/// omitted one means "all" off a terminal — a script that passed no filter
/// yesterday still reaches every Account today. On a terminal the same
/// absence is a question worth asking, so the picker offers
...
pub fn resolve_accounts(
    ...
    match filter {
        ...
        None if !is_tty || known.is_empty() => Ok(None),
        None => {
...
mod tests {
    ...
    fn an_unknown_account_names_an_empty_roster_as_such() {
        let err = resolve_accounts(Some("ghost@x.io".to_string()), &[], false).unwrap_err();
        assert!(format!("{err}").contains("no accounts"), "{err}");
    }

    #[test]
    fn explicit_known_account_narrows_to_it() {
        let filter = resolve_accounts(Some("b@x.io".to_string()), &known(), false).unwrap();
        assert_eq!(filter.as_deref(), Some("b@x.io"));
    }

    #[test]
    fn omitted_account_off_a_tty_means_all_accounts() {
        assert_eq!(resolve_accounts(None, &known(), false).unwrap(), None);
    }
    ...
    fn an_empty_roster_resolves_to_all_even_on_a_tty() {
        assert_eq!(resolve_accounts(None, &[], true).unwrap(), None);
    }
}
