...
:::tip
First deploy is single-pass and headless — `config.json` is templated and the superadmin is created via `gokapi --deployment-password` before the service starts. Caddy and DNS land in the same run. See [ADR-0009](https://github.com/espadat-studio/auberge/blob/master/meta/adr/0009-gokapi-headless-bootstrap.md).
:::

:::tip
Rotating `gokapi_admin_password` is not automatic. Delete the bootstrap marker on the host and redeploy — see [role README](https://github.com/espadat-studio/auberge/blob/master/ansible/roles/gokapi/README.md#rotating-the-admin-password).
:::
