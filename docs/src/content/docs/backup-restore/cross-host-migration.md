...
A host with `prefer_tailnet = true` is reached at its `tailscale_ip` and nothing falls back to the public address — see [Tailnet Transport](/configuration/tailnet-transport/). Headscale runs on `auberge`, so the tailnet route to the backup target depends on a service that target hosts, and a disaster recovery is exactly when that matters.
