...
This command has no `--output` flag. Output is always YAML. To get machine-readable JSON for a specific host, use `auberge host list --output json | jq '.[] | select(.name=="myserver")'`. See [ADR-0004](https://github.com/espadat-studio/auberge/blob/master/meta/adr/0004-cli-structured-output.md) for the reasoning.
