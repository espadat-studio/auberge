...
- An app in another DNS zone has its records there, not here, so `migrate` never sees them. `dns` resolves one zone per run (ADR-0081), so those apps are named in the `off_zone` array and on stderr — a report short of the roster would otherwise read as a completed migration. Migrating them is a second run, scoped to their zone.
...
`migrated` and `skipped` partition the records the run took as candidates, so a caller diffing against the zone can tell a record left behind on purpose from one the run never saw. `migrated` carries every in-scope record with the outcome of its write, failures included — read `success`, not membership. `off_zone` is neither: it names apps whose records are in a zone this run does not hold, so they were never candidates and no `old_ip` exists to report.
...
| `off_zone` | `app`       | string  | App whose Zone this run does not hold    |
| `off_zone` | `subdomain` | string  | Its effective subdomain                  |
| `off_zone` | `zone`      | string  | The Zone its record lives in             |

`migrated` and `skipped` rows appear in the order Cloudflare listed the zone; neither is sorted. `off_zone` is sorted by app, because it comes from the roster rather than the zone.
