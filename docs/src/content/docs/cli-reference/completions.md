...
Runtime values do not complete — `auberge deploy <TAB>` will not suggest App names, `-H <TAB>` will not suggest Hosts. Omit the value instead: the [interactive picker](/cli-reference/select/host/) prompts with fuzzy search.
...
See [Installation](/getting-started/installation/) for setup context.
